...
class CowsAndBullsController {
  ...
  getGuessAnswerHTML(guessedCode) {
    ...
    if (this.gameObject.guessSecretCode(guessedCode) == false) {
      //returns true if code is correct
      AnswerHTML = "<p>El codigo que ingreso no es el correcto</p>";
      AnswerHTML +=
        "<p>Pista obtenida:" +
        this.gameObject.getHintString(guessedCode) +
        "</p>";
      if (this.gameObject.getLifesRemaining() <= 0) {
        AnswerHTML +=
          "<p>Perdiste el juego, el codigo era: " +
          this.gameObject.getSecretCode() +
          "</p>";
        this.#setRestartButtonVisible();
      }
    } else {
      AnswerHTML +=
        "<p>Lo lograste!!!, el codigo que ingresaste es el correcto<p>";
