const valuesToGenerateSecretCode = [
  "0",
  "1",
  "2",
  "3",
...
class CowsAndBulls {
  ...
  getCowCharacters(code) {
    ...
    for (let index = 0; index < guessingCodeString.length; index++) {
      if (secretCodeString.includes(guessingCodeString[index])) {
        cowsCharacters += "*";
        secretCodeString = secretCodeString.replace(
          guessingCodeString[index],
          ""
        );
      }
    }
  ...
  generateCodeRandomly() {
    ...
    else numberOfValuesToGenerateCode = 9;
    for (let it = 0; it < this.codeLength; it++) {
      let randomIndex = Math.round(
        Math.random() * numberOfValuesToGenerateCode
      );
